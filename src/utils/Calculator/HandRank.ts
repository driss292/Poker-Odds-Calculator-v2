...
export class HandRankAlias {
  public static readonly HIGH_CARD: string = "HIGH_CARD";
  public static readonly PAIR: string = "PAIR";
  public static readonly TWO_PAIRS: string = "TWO_PAIRS";
  public static readonly TRIPS: string = "TRIPS";
  public static readonly STRAIGHT: string = "STRAIGHT";
  public static readonly FLUSH: string = "FLUSH";
  public static readonly FULL_HOUSE: string = "FULL_HOUSE";
  public static readonly QUADS: string = "QUADS";
  public static readonly STRAIGHT_FLUSH: string = "STRAIGHT_FLUSH";
}
...
export class HandRank {
  ...
  public static evaluate(game: IGame, cardgroup: CardGroup): HandRank {
    ...
    lodash.some(Object.keys(countBySuits), (suit: number) => {
    ...
    if (flushSuit > 0) {
      if (straightCardsCount >= 5) {
        const straightFlushCards: Card[] = lodash.filter(
          cardgroup,
          (card: Card): boolean => {
            return (
              card.getSuit() === flushSuit &&
              card.getRank() <= straightMaxCardRank
            );
          }
        );
        if (straightFlushCards.length >= 5) {
          ...
          for (let i: number = 1; i <= 4; i += 1) {
            if (
              straightFlushCards[i].getRank() !==
              straightFlushCards[i - 1].getRank() - 1
            ) {
          ...
          if (isStraightFlush) {
            return new HandRank(
              game.STRAIGHT_FLUSH,
              HandRankAlias.STRAIGHT_FLUSH,
              straightFlushCards.slice(0, 5)
            );
          }
        } else if (
          straightFlushCards.length === 4 &&
          // Five high straight (5-4-3-2-A)
          ((game.A2345_STRAIGHT &&
            straightFlushCards[0].getRank() === Rank.FIVE) ||
            // Five high straight (9-8-7-6-A)
            (game.A6789_STRAIGHT &&
              straightFlushCards[0].getRank() === Rank.NINE))
        ) {
          const aceCards: Card[] = lodash.filter(
            cardgroup,
            (card: Card): boolean => {
              return (
                card.getSuit() === flushSuit && card.getRank() === Rank.ACE
              );
            }
          );
          ...
          if (aceCards.length) {
            return new HandRank(
              game.STRAIGHT_FLUSH,
              HandRankAlias.STRAIGHT_FLUSH,
              straightFlushCards.concat(aceCards[0])
            );
          }
        }
      } else if (
    ...
    if (quadRanks.length === 1) {
      const quadCards: Card[] = lodash.filter(
        cardgroup,
        (card: Card) => card.getRank() === quadRanks[0]
      );
      const cards: Card[] = lodash.reject(
        cardgroup,
        (card: Card) => card.getRank() === quadRanks[0]
      );
      return new HandRank(
        game.QUADS,
        HandRankAlias.QUADS,
        quadCards.concat(cards).slice(0, 5)
      );
    }

    // Full house
    if (tripRanks.length === 1 && pairRanks.length >= 1) {
      const tripCards: Card[] = lodash.filter(
        cardgroup,
        (card: Card): boolean => {
          return card.getRank() === tripRanks[0];
        }
      );
      const pairCards: Card[] = lodash.filter(
        cardgroup,
        (card: Card): boolean => {
      ...
      return new HandRank(
        game.FULL_HOUSE,
        HandRankAlias.FULL_HOUSE,
        tripCards.concat(pairCards)
      );
      ...
      return new HandRank(
        game.FULL_HOUSE,
        HandRankAlias.FULL_HOUSE,
        tripCards.concat(pairCards.slice(0, 2))
      );
    }

    // Flush
    if (flushSuit > 0) {
      const flushCards: Card[] = lodash.filter(
        cardgroup,
        (card: Card): boolean => {
      ...
      return new HandRank(
        game.FLUSH,
        HandRankAlias.FLUSH,
        flushCards.slice(0, 5)
      );
    }

    // Straight
    if (straightCardsCount === 5) {
      const straightCards: Card[] = lodash.uniqWith(
        lodash.filter(cardgroup, (card: Card): boolean => {
      ...
      return new HandRank(
        game.STRAIGHT,
        HandRankAlias.STRAIGHT,
        straightCards.slice(0, 5)
      );
    } else if (
      straightCardsCount === 4 &&
      // Five high straight (5-4-3-2-A)
    ...
    if (tripRanks.length === 1) {
      ...
      const cards: Card[] = lodash.reject(cardgroup, (card: Card): boolean => {
        return card.getRank() === tripRanks[0];
      });
      return new HandRank(
        game.TRIPS,
        HandRankAlias.TRIPS,
        tripCards.concat(cards).slice(0, 5)
      );
    }

    // Two pairs
    if (pairRanks.length >= 2) {
      const pairedHigherCards: Card[] = lodash.filter(
        cardgroup,
        (card: Card): boolean => {
          return card.getRank() === pairRanks[0];
        }
      );
      const pairedLowerCards: Card[] = lodash.filter(
        cardgroup,
        (card: Card): boolean => {
          return card.getRank() === pairRanks[1];
        }
      );
      const unpairedCards: Card[] = lodash.reject(
        lodash.reject(
          cardgroup,
    ...
    if (pairRanks.length === 1) {
      const pairedCards: Card[] = lodash.filter(
        cardgroup,
        (card: Card): boolean => {
          return card.getRank() === pairRanks[0];
        }
      ...
      return new HandRank(
        game.PAIR,
        HandRankAlias.PAIR,
        pairedCards.concat(unpairedCards).slice(0, 5)
      );
    }
  ...
  public compareTo(handrank: HandRank): number {
    if (this.getRank() === handrank.getRank()) {
      for (let i: number = 0; i < 5; i += 1) {
        if (
          this.getHighCards()[i].getRank() !==
          handrank.getHighCards()[i].getRank()
        ) {
          return this.getHighCards()[i].getRank() >
            handrank.getHighCards()[i].getRank()
            ? 1
            : -1;
        }
  ...
  public toString(): string {
    ...
    switch (this.alias) {
      case HandRankAlias.STRAIGHT_FLUSH:
        if (this.highcards[0].getRank() === Rank.ACE) {
          ...
          s =
            lodash.capitalize(this.highcards[0].toString(false, true)) +
            " high straight flush";
        }
        break;
      ...
      case HandRankAlias.FULL_HOUSE:
        s = `Full house: ${this.highcards[0].toString(
          false,
          true,
          true
        )} full of ${this.highcards[4].toString(false, true, true)}`;
        break;
      ...
      case HandRankAlias.TWO_PAIRS:
        s = `Two pairs: ${this.highcards[0].toString(
          false,
          true,
          true
        )} and ${this.highcards[2].toString(false, true, true)}`;
        showHighcards = 1;
        break;
    ...
    if (showHighcards > 0) {
      const highcards: string[] = this.highcards
        .slice(5 - showHighcards, 5)
        .map((h: Card) => {
