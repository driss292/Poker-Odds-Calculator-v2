...
export class HandEquity {
  ...
  public getEquity(): number {
    ...
    return (this.bestHandCount * 100.0) / this.possibleHandsCount;
  }
  ...
  public getTiePercentage(): number {
    ...
    return (this.tieHandCount * 100.0) / this.possibleHandsCount;
  }
...
export class OddsCalculator {
  public static readonly DEFAULT_ITERATIONS: number = 100000;
  ...
  public static calculate(
    ...
    const allGroups: CardGroup[] = board
      ? cardgroups.concat(board)
      : cardgroups;
    let allCards: Card[] = [];
    ...
    if (deadCards) {
      allCards = allCards.concat(deadCards);
    }
    ...
    const uniqCards: Card[] = lodash.uniqBy(allCards, (card: Card) => {
    ...
    handranks = cardgroups.map((cardgroup: CardGroup): HandRank => {
      return HandRank.evaluate(
        game,
        board ? cardgroup.concat(board) : cardgroup
      );
    });

    const equities: HandEquity[] = cardgroups.map((): HandEquity => {
      return new HandEquity();
    });

    const selectWinners = (simulatedBoard: CardGroup) => {
    ...
    if (!board || board.length === 0) {
      ...
      for (let x: number = iterations; x > 0; x -= 1) {
        const index1: number = lodash.random(0, remainingCount - 1);
        let index2: number;
        let index3: number;
        let index4: number;
        let index5: number;
        ...
        do {
          index2 = lodash.random(0, remainingCount - 1);
        } while (index2 === index1);

        do {
          index3 = lodash.random(0, remainingCount - 1);
        } while (index3 === index1 || index3 === index2);

        do {
          index4 = lodash.random(0, remainingCount - 1);
        } while (index4 === index1 || index4 === index2 || index4 === index3);

        do {
          index5 = lodash.random(0, remainingCount - 1);
        } while (
          index5 === index1 ||
          index5 === index2 ||
          index5 === index3 ||
      ...
      for (const c of remainingCards) {
        const simulatedBoard: CardGroup = board.concat(
          CardGroup.fromCards([c])
        );
        iterations += 1;
        selectWinners(simulatedBoard);
      }
      ...
      for (let a: number = 0; a < remainingCount; a += 1) {
        for (let b: number = a + 1; b < remainingCount; b += 1) {
          const simulatedBoard: CardGroup = board.concat(
            CardGroup.fromCards([remainingCards[a], remainingCards[b]])
          );
          iterations += 1;
          selectWinners(simulatedBoard);
        }
    ...
    return new OddsCalculator(
      equities,
      handranks,
      iterations,
      jobEndedAt - jobStartedAt
    );
  }
